...
const getSectionSummary = (title?: string) => {
  if (!title) return '';
  switch (title) {
    case 'Objectives': return 'Defines the key business objectives, expected outcomes, and the overall purpose of the proposed solution.';
    case 'Executive Summary': return 'Provides a high-level overview of the project, including its purpose, goals, and key deliverables.';
    case 'Project Scope': return 'Outlines the specific boundaries, deliverables, and features that are included or excluded from the project.';
    case 'Solution Architecture': return 'Describes the technical design, framework, and infrastructure required to implement the proposed solution.';
    case 'Technical Requirements': return 'Details the hardware, software, and performance specifications necessary for the project.';
    case 'Deliverables': return 'Lists the tangible outputs, documents, or products that will be provided upon project completion.';
    case 'Timeline': return 'Specifies the project schedule, including key milestones, phases, and estimated completion dates.';
    case 'Commercial Proposal': return 'Provides the pricing structure, payment terms, and overall cost breakdown for the proposed services.';
    case 'Risks & Assumptions': return 'Identifies potential project risks and the underlying assumptions made during project planning.';
    case 'Acceptance Criteria': return 'Defines the specific conditions and metrics that must be met for the deliverables to be accepted.';
    default: return 'Provides detailed information, guidelines, and specifications relevant to this particular section of the document.';
  }
};
...
export function SOWDraft({ 
  ...
  const [saveTooltipStyle, setSaveTooltipStyle] = useState<{top?: number, bottom?: number, left?: number, width?: number}>({});
  const [saveTooltipArrowStyle, setSaveTooltipArrowStyle] = useState<any>({});
  ...
  const handleSaveMouseEnter = () => {
    if (saveBtnRef.current) {
      ...
      setSaveTooltipArrowStyle({
        [isTop ? 'bottom' : 'top']: '-6px',
        left: arrowLeft,
        borderBottom: isTop ? '1px solid var(--app-color-border)' : 'none',
        borderRight: isTop ? '1px solid var(--app-color-border)' : 'none',
        borderTop: !isTop ? '1px solid var(--app-color-border)' : 'none',
        borderLeft: !isTop ? '1px solid var(--app-color-border)' : 'none'
      });
    }
  ...
  // Real-time reviewer sync listener
  useEffect(() => {
    const handleStorage = (e: StorageEvent) => {
      if (e.key === 'sow-section-updated' && e.newValue) {
        try {
          const data = JSON.parse(e.newValue);
          const currentSectionName = tocItems[activeSectionIndex]?.title;
          const currentUser = userRole === 'Reviewer' ? 'David Brown' : 'Dipali Balkrishna Patil';
          const isShared = (tocItems[activeSectionIndex]?.assignedReviewers?.length || 0) > 1;
          
          if (data.sectionName === currentSectionName && data.user !== currentUser && isShared) {
            setShowSyncNotification(true);
            setSyncTimestamp(data.timestamp);
          }
        } catch (error) {
  ...
  useEffect(() => {
    const lastUpdateStr = localStorage.getItem('sow-section-updated');
    if (lastUpdateStr) {
      try {
        const data = JSON.parse(lastUpdateStr);
        const currentUser = userRole === 'Reviewer' ? 'David Brown' : 'Dipali Balkrishna Patil';
        const isShared = (tocItems[activeSectionIndex]?.assignedReviewers?.length || 0) > 1;
        
        if (data.sectionName === tocItems[activeSectionIndex]?.title && data.user !== currentUser && isShared && (Date.now() - data.timestamp < 5 * 60 * 1000)) {
           setTimeout(() => {
             setToastMessage('This section has been updated with the latest changes.');
  ...
  const [rejectPopupSection, setRejectPopupSection] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        if (rejectPopupSection !== null) setRejectPopupSection(null);
  ...
  const exportBlock = (
    <div style={{ position: 'relative', display: 'flex', gap: '12px' }}>
      <Button variant="accent" onClick={handleExportWord} disabled={isProcessing}>
    ...
    <div className="page-container" style={{ 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', minHeight: '24px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'flex-end' }}>
                            {isApprovedMode ? (
                              ...
                              <span title="Approved" style={{ display: 'flex' }}><Icon name="check-circle" size={18} style={{ color: 'var(--app-color-success)' }} /></span>
                            ) : (item.status === 'Rejected' || item.status === 'Rework Required') ? (
                              <span title="Rejected" style={{ display: 'flex' }}><Icon name="x-circle" size={18} style={{ color: 'var(--app-color-danger)' }} /></span>
                            ) : (
                              <>
                                <div 
                                ...
                                {(!item.assignedReviewers || item.assignedReviewers.length === 0) && userRole !== 'Reviewer' && !isApprovedMode && item.status !== 'Approved' && item.status !== 'Rejected' && item.status !== 'Rework Required' && (
                                  ...
                                  >
                                    <div style={{
                                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                                      cursor: 'pointer', color: 'var(--app-color-primary)',
                                      padding: '4px', borderRadius: '4px',
                                      transition: 'background-color 0.2s',
                                      backgroundColor: 'transparent'
                                    }}
                                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--app-color-primary-soft)'}
                                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                                    >
                                  ...
                                  <>
                                    ...
                                    <div className="context-menu-animate" style={{
                                      ...
                                      {userRole !== 'Reviewer' && !isReviewMode && !isApprovedMode && (
                                        <button 
                                          onClick={(e) => { 
                                            ...
                                            e.stopPropagation(); setAddReviewerSectionId(item.title); setNewReviewerSections([item.title]); setOpenTocMenuIndex(null); 
                                          }}
                                          style={{
                                      ...
                                      <button 
                                        onClick={(e) => { 
                                          if (item.status === 'Rejected' || item.status === 'Rework Required') return;
                                          e.stopPropagation(); setApprovePopupSection(idx); setOpenTocMenuIndex(null); 
                                        }}
                                        style={{
                                          display: 'flex', alignItems: 'center', gap: '14px', width: '100%', padding: '10px 12px',
                                          backgroundColor: 'transparent', border: 'none', borderRadius: '4px',
                                      ...
                                      {userRole !== 'Reviewer' && (
                                        <button 
                                          onClick={(e) => { 
                                            if (item.status === 'Rejected' || item.status === 'Rework Required') return;
                                            e.stopPropagation(); setRejectPopupSection(idx); setOpenTocMenuIndex(null); 
                                          }}
                                          style={{
                                            display: 'flex', alignItems: 'center', gap: '14px', width: '100%', padding: '10px 12px',
                                            backgroundColor: 'transparent', border: 'none', borderRadius: '4px',
                                          ...
                                          disabled={item.status === 'Rejected' || item.status === 'Rework Required'}
                                        >
                                          <Icon name="corner-up-left" size={18} /> Rework for all Reviewers
                                        </button>
                                      )}
                                    </div>
                                  </>
      ...
      {toastMessage && (
        <div style={{
          ...
          <Icon 
            name={toastMessage.toLowerCase().includes('reject') ? 'x-circle' : 'check-circle'} 
            size={18} 
            style={{ color: toastMessage.toLowerCase().includes('reject') ? 'var(--app-color-danger)' : 'var(--app-color-success)' }} 
          />
          {toastMessage}
        </div>
      )}
          ...
          <div style={{
            ...
            <div style={{ padding: '24px 24px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <p style={{ margin: 0, fontSize: '14px', color: 'var(--app-color-text-muted)', lineHeight: 1.5 }}>
                You have unsaved changes in this section. Do you want to save before leaving?
              </p>
            </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ width: '100%', textAlign: 'left' }}>
                ...
                <textarea 
                  ...
                  placeholder="Add an approval comment or note (optional)..."
                  style={{
            ...
            <div style={{ padding: '0 24px 24px', display: 'flex', justifyContent: 'flex-end', gap: '12px', width: '100%' }}>
              ...
              <Button variant="accent" onClick={() => {
                ...
                setApprovePopupSection(null);
                setApprovalComment('');
                setToastMessage('Section approved.');
                setTimeout(() => setToastMessage(''), 3000);
      ...
      {/* Reject Section Confirmation Modal */}
      {rejectPopupSection !== null && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: 'rgba(13, 33, 44, 0.4)',
          backdropFilter: 'blur(4px)',
          ...
          <div style={{
            ...
            {/* Header */}
            <div style={{ padding: '24px', display: 'flex', alignItems: 'center', borderBottom: '1px solid var(--app-color-border)' }}>
              <Icon name="corner-up-left" size={20} style={{ color: 'var(--app-color-text)', marginRight: '12px' }} />
              <h2 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--app-color-text)', margin: 0 }}>Rework for all Reviewers</h2>
              <button 
                onClick={() => setRejectPopupSection(null)}
                style={{ marginLeft: 'auto', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '4px' }}
              >
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ width: '100%', textAlign: 'left' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: 'var(--app-color-text)', marginBottom: '8px' }}>
                  Rework Instructions <span style={{ color: 'var(--app-color-danger)' }}>*</span>
                </label>
                <textarea 
                  value={approvalComment}
                  onChange={(e) => setApprovalComment(e.target.value)}
            ...
            <div style={{ padding: '0 24px 24px', display: 'flex', justifyContent: 'flex-end', gap: '12px', width: '100%' }}>
              <Button variant="ghost" onClick={() => {
                setRejectPopupSection(null);
                setApprovalComment('');
              }} style={{ border: '1px solid var(--app-color-border)', padding: '8px 24px' }}>
                Cancel
              </Button>
              <Button variant="accent" disabled={!approvalComment.trim()} onClick={() => {
                if (rejectPopupSection === null) return;
                const newItems = [...tocItems];
                newItems[rejectPopupSection].status = 'Rejected';
                setTocItems(newItems);
                ...
                const newActivity: ActivityItem = {
                  ...
                  user: userRole === 'Reviewer' ? 'David Brown' : 'Dipali Balkrishna Patil',
                  timestamp: new Date(),
                  metadata: { approvalComment: approvalComment.trim() }
                };
                setActivityLog(prev => [newActivity, ...prev]);
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Icon name="corner-up-left" size={16} /> Rework for all Reviewers
                </div>
              </Button>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '0 24px 24px', display: 'flex', justifyContent: 'flex-end', gap: '12px', width: '100%' }}>
              ...
              <Button 
                ...
                // Reset the status of the section so it can be approved/rejected again
                const newItems = [...tocItems];
                newItems[regeneratePopupSection].status = undefined;
                ...
                setTimeout(() => {
                  setTocItems(newItems);
                  setToastMessage('Section regenerated.');
                  setTimeout(() => setToastMessage(''), 3000);
                }, 4400);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div key="gen" className="fade-in-content" style={{ textAlign: 'center', marginBottom: '16px' }}>
              <div style={{ width: '40px', height: '40px', backgroundColor: 'var(--app-color-primary-soft)', color: 'var(--app-color-primary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ height: '6px', backgroundColor: 'var(--app-color-surface-muted)', borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{ height: '100%', backgroundColor: 'var(--app-color-primary)', width: `${processingProgress}%`, transition: 'width 0.1s linear' }} />
              </div>
                ...
                {processingStages.map((stage, idx) => {
                  ...
                  return (
                    <li key={idx} style={{
                      ...
                      fontSize: '13px',
                      color: isCompleted ? 'var(--app-color-text)' : isCurrent ? 'var(--app-color-primary)' : 'var(--app-color-text-muted)',
      ...
      {(isDraftMode || isReviewMode || isApprovedMode) && document.getElementById('topbar-right-portal-target') && createPortal(
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', animation: 'fadeIn 0.3s ease-out' }}>
            {/* Assign Reviewer Button (PMO only) */}
            {userRole === 'PMO' && !isApprovedMode && (
              <Button variant="secondary" onClick={() => { setAddReviewerSectionId('all'); setNewReviewerSections(sections.filter(s => s.included).map(s => s.name)); }} disabled={isProcessing}>
                <Icon name="user-plus" size={16} /> Assign Reviewer
              </Button>
            )}

            {/* Share Button (PMO only) */}
            {userRole === 'PMO' && !isApprovedMode && (
      ...
      <Toast 
        ...
        action={
          hasUnsavedChanges ? (
            <>
              <Button variant="primary" onClick={handleSaveAndRefresh} style={{ padding: '6px 12px', fontSize: '13px' }}>
